...
pub async fn prepare_order(
    ...
    for item in items.into_iter() {
        let usd_price = get_product(ctx, item.product_id.clone()).await.price_usd;
        let converted_price = convert_currency(ctx, usd_price, user_currency.clone()).await;
        order.push(OrderItem {
            item,
            cost: converted_price,
        });
    }
    order
}
